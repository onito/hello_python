 ...
 Understanding UNICODE & encoding UTF-8
  - it only can work in 'Console' <f5>,
    ...
    for detail : published 2016.3/2 - understanding UNICODE
        ...
        https://www.slideshare.net/LeeSeongjoo/2-17395073?next_slideshow=1
...
def test2_ascii_unicode(start, end):
    """
    ...
    for n in range(start, end):
        ...
        if column < 6:
            ...
            column = 0
            print('{:3}={}  '.format(n, letter), end='\n')
# test2_ascii_unicode(0xac00, 0xacff)
# test2_ascii_unicode(0x00, 0x7f)
# test2_ascii_unicode(0x80, 0xff)

def test3_unicode_utf8_test():
    print(sys.getdefaultencoding())
    print(sys.stdout.encoding)
    print(sys.stderr.encoding)
    print('안녕세계 --> Hello World')
# test3_unicode_utf8_test()
...
def compare_lambda_vs_function(x, y):
    _a = (lambda x, y: x**2+y**2)(x,y)
    print('lambda   = %s'% _a)

    def tempo(x,y):
        return (x**2 + y**2)
    _b = tempo(x, y)
    print('function = %s'% _b, end='\n\n')
# compare_lambda_vs_function(4, 3)
# compare_lambda_vs_function(2, 1)
# compare_lambda_vs_function(5, 7)
# compare_lambda_vs_function(6, 9)

def test9_very_big_map_object():
    _x = [n for n in range(1, 100, 2)]
    _y = [i for i in range(11, 110, 2)]
    _b = map(compare_lambda_vs_function, _x, _y)
    print(list(_b))
    print(len(_x), len(_y))
# test9_very_big_map_object()
