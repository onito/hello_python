...
def get2_answer_n_checksum():     # OUT= 'int', 'bool'
    while True:
        ...
        print('The CHECKSUM is....', _checksum, end=", ")
        print(len(ANSWER_ARR)+1,'th trial..')
        print('\n\n')

        if _checksum:
            if not int(_answer) in ANSWER_ARR:
                ANSWER_ARR.append(int(_answer))
...
def main_loop(_my_number):
    while True:
        ...
        if _answer > _my_number:
            ...
            print('===========')
            print("Huh?.. BULL'S EYE!!!.. My number is... %s" %_my_number)
